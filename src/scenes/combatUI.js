...
export class combatUI extends Phaser.Scene {
  ...
  selectGobbo(target){
    this.movementGrid = this.add
      .renderTexture(0,0,32,64)
      .setInteractive()
  ...
  create() {
    ...
    this.stitch = this.add
      .renderTexture(0,0,32,42)
      .setInteractive();
      // Draws the two sprites as one at location
    this.stitch.draw([this.chungBody,this.chungHead],
    ...
    this.stitch.on('pointerdown', () =>
          this.selectGobbo(this.chung))
    
    this.input.on('pointerdown', () =>
          this.movementConfirm(this.chung))
  ...
  update() {
    ...
    this.stitch.x = this.currentMap.tileToWorldXY(this.chung.xPos,this.chung.yPos).x-16;
    this.stitch.y = this.currentMap.tileToWorldXY(this.chung.xPos, this.chung.yPos).y-36;
